...
| `related_acts` | A metadata field for confirmed, source-backed local relationships that help the graph connect acts. | The graph generator treats entries as confirmed weak `related` edges, so do not place suggested or inferred edges here. |
| `relationships[]` | Optional structured metadata records for graph-visible edges that need confidence or evidence annotation. | Use this path for `suggested` or `inferred` edges, or for confirmed edges where the evidence fields should travel with the metadata. |
| `confidence` | A structured relationship label: `confirmed`, `suggested`, or `inferred`. | In generated graph output, `confirmed` becomes `review_status: confirmed`; `suggested` and `inferred` become `needs_review`. |
| `evidence_type` | A structured relationship field describing why an edge exists, such as explicit text, Portal action, cross-reference, structural review, inference, or manual review. | `confidence: confirmed` cannot be paired with `evidence_type: inferred`. |
...
| citation anchor | A stable Markdown anchor used to point to a specific article, technical provision, or annex in a full-text act page. | Supported canonical targets are `#art-N`, `#pct-N` (including nested provisions such as `#pct-1-1`), and `#anexa-N`; anchors must follow official numbering and remain unique. |
...
| `needs_review` | A generated graph-edge review status for a relationship that has not been confirmed as source-backed metadata. | `cross-references/relationships-auto.json` suggestions and structured `relationships[]` records with `confidence: suggested` or `confidence: inferred` are emitted as `needs_review`. |
