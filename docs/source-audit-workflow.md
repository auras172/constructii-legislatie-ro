...
For handling changed source hashes, defacement signals, and quarantine
decisions, see [`docs/source-drift-runbook.md`](source-drift-runbook.md).
...
| `authority` | Source classification: `Portal Legislativ`, `Monitorul Oficial`, `MDLPA`, `ISCIR`, `ANRE`, `IGSU`, `ASRO`, `Ministerul Mediului`, `ANAP`, `ISC`, or `other` |
...
2. verifies that the selected source URL belongs to an approved official-source
   ...
   `iscir.ro`, `anre.ro`, `igsu.ro`, `asro.ro`, `mmediu.ro`,
   `anap.gov.ro`, or `isc.gov.ro`);
...
The allowlist intentionally includes official technical and institutional
sources beyond primary legislation portals. For example, ASRO URLs may support
metadata about referenced standards, but the workflow does not grant permission
to copy copyrighted standard text into this repository.
